...
export function useSettings(user: any) {
  const [settings, setSettings] = useState<any>({ monthlyGoal:0, estimatedMonthlyPoints:0, lastGeneratedDate:null })
  useEffect(() => {
    if (!user) return
    const ref = doc(db, `artifacts/${appId}/users/${user.uid}/settings/main`)
    return onSnapshot(ref, s => { if (s.exists()) setSettings({...settings, ...s.data()}) })
  }, [user?.uid])
...
export function useTasks(user: any, depKey?: string) {
  ...
  useEffect(() => {
    ...
    return onSnapshot(coll, snap => setTasks(snap.docs.map(d => ({ id: d.id, ...d.data() } as any))))
  }, [user?.uid, depKey])
  return tasks
}
